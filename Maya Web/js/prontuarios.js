...
export function renderizarProntuarios(db, mainContent) {
    ...
    // 2. Busca os pacientes na coleção "Usuarios" para preencher a caixa de seleção
    ...
    onSnapshot(pacientesRef, (snapshot) => {
        ...
        if (!selectElement) return; // Evita erro se a tela for fechada antes de carregar

        let optionsHtml = '<option value="">Selecione o paciente atendido...</option>';
        snapshot.forEach((doc) => {
            const dados = doc.data();
            // Tenta pegar o nome pelo campo que vocês configuraram no Java
    ...
    // 3. Lógica para capturar o clique em "Salvar" e enviar para o Firebase
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            // Cria a coleção "Prontuarios" e insere um novo documento
